...
import { CheckCircle2, Eye, EyeOff, Mail, Moon, Sun } from 'lucide-react';
...
const Register = () => {
  ...
  return (
    <AuthShell
      ...
      subtitle={language === 'id' ? 'Daftar untuk mulai mencatat transaksi dengan layout yang lebih rapi dan ringan.' : 'Sign up to start tracking transactions with the cleaner and lighter layout.'}
      heroTitle={language === 'id' ? 'Mulai susun catatan keuangan yang lebih terarah' : 'Start building a more organized financial record'}
      heroDescription={language === 'id' ? 'Buat akun untuk mengelola pemasukan, pengeluaran, dan ringkasan bulanan dengan tampilan yang tetap selaras dengan dashboard.' : 'Create an account to manage income, expenses, and monthly summaries in a view that stays aligned with the dashboard.'}
      topAction={(
        <button
          type="button"
          onClick={toggleTheme}
      ...
      footer={(
        <div className="space-y-5">
          ...
          <div className="text-sm">
            ...
            <Link to="/login" className={cn('font-semibold', isDark ? 'text-amber-300 hover:text-amber-200' : 'text-blue-700 hover:text-blue-600')}>
      ...
      <div className="space-y-6">
        ...
        {success && (
          <Alert className="rounded-2xl border-emerald-500/30 bg-emerald-500/10">
            <AlertDescription className="flex items-center gap-2 text-emerald-600 dark:text-emerald-300">
              <CheckCircle2 className="h-4 w-4" />
              {language === 'id' ? 'Pendaftaran berhasil. Mengalihkan ke login...' : 'Registration successful. Redirecting to sign in...'}
            </AlertDescription>
          </Alert>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-2">
            <Label htmlFor="confirm-password" className={cn('text-sm font-medium', isDark ? 'text-zinc-200' : 'text-slate-700')}>
